...
import {ReportCeoReportAction} from "../../store/CeoReport/action";

class ReportHopeToCeoByCeoScreen extends React.Component {
    constructor(props) {
        super(props)
        ...
        this.state = {
            ...
            report_id: this.props.route.params.report_id,
    ...
    onPressReport = () => {
        const {textMsg, isChecked, isChecked2, report_id, customer_id} = this.state
        ...
        this.props.ReportCeoReport(sendData)
    }
    ...
    onPressModalOk() {
        ...
        if (currentscreen === "CeoShopManagerScreen") {
            this.props.navigation.navigate("CeoShopManagerScreen", {shop_id, tabselect: 3});  //  뉴스/이벤트 태브로 초기화
        } else
            this.props.navigation.navigate("CeoShopManagerScreen");  //  뉴스/이벤트 태브로 초기화
    }
...
const mapStateToProps = (state) => {
    return {
        ...
        isReportLoading: state.ceoreport.isReportLoading,
        errCode: state.ceoreport.err,
        errMsg: state.ceoreport.message,
    };
};

const mapDispatchToProps = (dispatch) => {
    return {
        ReportCeoReport: (data) => dispatch(ReportCeoReportAction(data)),
    };
};

export default connect(mapStateToProps, mapDispatchToProps)(ReportHopeToCeoByCeoScreen);
